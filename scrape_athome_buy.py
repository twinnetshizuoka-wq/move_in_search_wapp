...
from scraper.athome import ScrapeError, scrape_current_session, set_scraper_config, wait_for_user_start
from scraper.config import KOUNYU_CONFIG

DEFAULT_DATA_DIR = Path("data")
ATHOME_BUY_TOP_URL = KOUNYU_CONFIG.top_url


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """コマンドライン引数を解析する。"""
    parser = argparse.ArgumentParser(
        description="アットホームの購入物件一覧から物件情報を半自動取得します。",
    )
    parser.add_argument(
        "-o",
        "--output",
...
def run_scraper(output_csv: Path | None) -> tuple[int, Path]:
    ...
    set_scraper_config(KOUNYU_CONFIG)
    ...
    with sync_playwright() as playwright:
        ...
        try:
            print(f"アットホーム購入トップを開きます: {ATHOME_BUY_TOP_URL}")
            page.goto(ATHOME_BUY_TOP_URL, wait_until="domcontentloaded", timeout=60000)
            ...
            listings = scrape_current_session(
                page,
                output_csv=resolved_output,
                config=KOUNYU_CONFIG,
            )
