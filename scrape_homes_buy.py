...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="LIFULL HOME'S の購入物件一覧から物件情報を半自動取得します。",
    )
    parser.add_argument(
        "-o",
        "--output",
        ...
        default=None,
        help=(
            "出力 CSV のパス（省略時は data/hs_kounyu_都道府県_市区町村_YYYYMMDDHHMM.csv を自動生成）"
        ),
    )
    if argv is None:
...
def run_scraper(output_csv: Path | None) -> tuple[int, Path]:
    ...
    with sync_playwright() as playwright:
        ...
        try:
            print(f"HOME'S 購入トップを開きます: {HOMES_BUY_TOP_URL}")
            page.goto(HOMES_BUY_TOP_URL, wait_until="domcontentloaded", timeout=60000)
